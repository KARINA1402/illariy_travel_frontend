...
export class ReservasRegisterComponent implements OnInit {
  ...
  // Caché de paquetes para validaciones
  ...
  // ─── MÉTODOS DE UI (MODALES, SELECCIONES) ──────────────────────────
  ...
  newReservaArray(detalle: DetalleReservaModel): FormGroup {
    const paqueteEncontrado = this.paquete.find(
      (p) => p.iD_Paquete === detalle.iD_Paquete
    );
    const destino_nombre = paqueteEncontrado
      ? this.destino_nombreTipoMap.get(paqueteEncontrado.iD_Destino)
      : '';
    const destino_moneda = paqueteEncontrado
      ? this.destino_monedaTipoMap.get(paqueteEncontrado.iD_Destino)
      : '';
  ...
  agregarAcompanante(cliente: ClienteModel) {
    if (this.acompanantes.find((a) => a.iD_Cliente === cliente.iD_Cliente)) {
      Swal.fire({
        position: 'center',
        icon: 'warning',
        title: 'Este cliente ya está agregado',
    ...
    if (this.obtenerAcompanantesFaltantes() <= 0) {
      Swal.fire({
        position: 'center',
        icon: 'warning',
        title: 'Ya se alcanzó el número máximo de personas',
        showConfirmButton: false,
        timer: 1500,
      });
      return;
    ...
    this.acompanantesList = this.acompanantesList.filter(
  ...
  obtenerAcompanantesFaltantes(): number {
    const numeroPersonas =
      parseInt(this.myForm.get('numero_Personas')?.value) || 1;
  ...
  // ─── MÉTODOS DE CÁLCULO (TOTAL, VUELTO) ─────────────────────────────
  ...
  calcularTotalPrice() {
    ...
    if (this.paqueteselect?.iD_Paquete) {
      this.nombre_moneda = this.destino_monedaTipoMap.get(
        this.paqueteselect.iD_Destino
      ) || '';
    }
  ...
  // ─── MÉTODOS DE VALIDACIÓN DE FECHAS Y CONFLICTOS ──────────────────
  ...
  private esDuplicado(
    paqueteNuevoId: number,
    inicioNuevo: Date,
    finNuevo: Date,
    paqueteExistenteId: number,
    inicioExistente: Date,
    finExistente: Date
  ): boolean {
    return (
      paqueteNuevoId === paqueteExistenteId &&
      inicioNuevo.getTime() === inicioExistente.getTime() &&
      finNuevo.getTime() === finExistente.getTime()
    );
  }

  private async verificarConflictoReserva(): Promise<{
    conflict: boolean;
    tipo: 'duplicado' | 'cruce' | null;
  }> {
    const clienteId = this.normalizeId(this.clienteSelect.iD_Cliente);
    if (!clienteId) return { conflict: false, tipo: null };

    const reservas = await firstValueFrom(this._reservasService.getAll());
    const reservasCliente = reservas.filter(
      (r) => this.normalizeId(r.iD_Cliente) === clienteId
    );

    console.log('reservasCliente encontradas:', reservasCliente);
    if (reservasCliente.length === 0) return { conflict: false, tipo: null };

    const nuevoPaquete = this.paqueteselect;
    const nuevoPaqueteId = this.normalizeId(nuevoPaquete.iD_Paquete);
    ...
    if (!nuevaFechaInicio || !nuevaFechaFin) {
      console.warn(
        'No se pudieron parsear las fechas del nuevo paquete:',
        nuevoPaquete.fecha_Inicio,
        nuevoPaquete.fecha_Fin
      );
      return { conflict: false, tipo: null };
    }

    let allPaquetes = this.paquetesCache;
    if (!allPaquetes || allPaquetes.length === 0) {
      allPaquetes = await firstValueFrom(this._produtoservice.getAll());
      this.paquetesCache = allPaquetes;
    }

    for (const reserva of reservasCliente) {
      const idPaqueteReserva = this.normalizeId(reserva.iD_Paquete);
      const paqueteReserva = allPaquetes.find(
        (p) => this.normalizeId(p.iD_Paquete) === idPaqueteReserva
      );
      ...
      if (!paqueteReserva) {
        console.warn(
          '⚠️ No se encontró el paquete',
          idPaqueteReserva,
          'en el catálogo. Revisa el nombre del campo en la respuesta de GET /api/Reserva.'
        );
        continue;
      }
      ...
      if (
        ...
        console.log('🔴 DUPLICADO detectado con reserva:', reserva);
        return { conflict: true, tipo: 'duplicado' };
      }

      // Chequeo de cruce/margen insuficiente
      if (
        this.fechasSeCruzan(
          nuevaFechaInicio,
          nuevaFechaFin,
        ...
        console.log('🔴 CRUCE DE FECHAS detectado con reserva:', reserva);
        return { conflict: true, tipo: 'cruce' };
      }
    }
  ...
  // ─── MÉTODO PRINCIPAL: REALIZAR RESERVA ─────────────────────────────

  async realizarReserva(template: TemplateRef<any>) {
    console.log(
      '%c🔴 VERSION NUEVA - realizarReserva EJECUTADO',
      'background: red; color: white; font-size: 20px; padding: 5px;'
    );
    ...
    if (this.obtenerAcompanantesFaltantes() > 0) {
      Swal.fire({
        icon: 'warning',
        title: `Faltan ${this.obtenerAcompanantesFaltantes()} acompañante(s)`,
      });
      return;
    }
    ...
    // 2. Validación de duplicados y cruces de fechas
    try {
      const resultado = await this.verificarConflictoReserva();
      if (resultado.conflict) {
        if (resultado.tipo === 'duplicado') {
          Swal.fire({
            icon: 'error',
            title: 'Reserva duplicada',
            text: 'Este cliente ya tiene una reserva idéntica: mismo paquete y mismas fechas.',
      ...
      console.error('Error al verificar conflictos de reserva:', error);
      Swal.fire({
        icon: 'warning',
    ...
    const numeroTransaccion = `RES-${Date.now()}-${Math.floor(
      Math.random() * 1000
    )}`;
    ...
    const payload = {
      ...
      DestinoNombre:
        this.destino_nombreTipoMap.get(this.paqueteselect.iD_Destino) || '',
      Cliente: {
        ID_Cliente: this.clienteSelect.iD_Cliente,
